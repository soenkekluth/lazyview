...
var LazyView = function (_EventDispatcher) {
  ...
  LazyView.prototype.render = function render() {
    if (this.options.enterClass || this.options.exitClass) {
      ...
      this.el.className = (0, _dedupe2.default)(this.el.className, (_classNames = {}, _classNames[this.options.enterClass] = this.state.inView, _classNames[this.options.exitClass] = !this.state.inView, _classNames));
    }
  };
  ...
  LazyView.prototype.onScroll = function onScroll() {
    ...
    if (this.offsetKeys) {
      ...
      for (var _i = 0, _l = this.offsetKeys.length; _i < _l; _i++) {
        var key = this.offsetKeys[_i];
  ...
  LazyView.prototype.isInView = function isInView() {
    var offsetPosition = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : nullPosition;

    if (this.state.position === 'fixed') {
      return true;
    }
    var progress = this.getProgress(offsetPosition);
  ...
  LazyView.prototype.getProgress = function getProgress() {
    var offsetPosition = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : nullPosition;

    var posY = this.position.top - this.scroll.clientHeight + offsetPosition.top;
  ...
  LazyView.prototype.cachePosition = function cachePosition() {
    ...
    if (!!this.options.childrenSelectors && this.options.childrenSelectors.length) {
      var query = this.options.childrenSelectors.join(', ');
    ...
    if (selectedChildren) {
      ...
      for (var _i2 = 0, _l2 = selectedChildren.length; _i2 < _l2; _i2++) {
        var _rect = getAbsolutBoundingRect(selectedChildren[_i2]);
        _rect.top -= this.position.top;
        _rect.bottom -= this.position.top;

        this.children.push({
          el: selectedChildren[_i2],
          position: _rect,
          state: {
            inView: false
          }
...
LazyView.EXIT = 'exit';
LazyView.ENTER_CHILD = 'enter:child';
LazyView.EXIT_CHILD = 'exit:child';
